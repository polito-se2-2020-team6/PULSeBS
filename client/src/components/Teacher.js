...
class Teacher extends React.Component {
  ...
  changeRange(x, courseId) {
    ...
    var lung = this.state.totalLectures.filter((l) => l.courseName === courseId)
      .length;
    if (x < 0) {
      if (this.state.range - 1 >= 0) {
        range--;
      }
    } else if (x > 0) {
      if (this.state.range + 1 < Math.ceil(lung / view)) {
        range++;
      }
    }
                    ...
                    <Tab eventKey={C_Id} title={C_Id} key={C_Id}>
                      <Row className="mt-5 ">
                        ...
                        <Col md={5}>
                          <Pagination id="pagId">
                            <Pagination.Prev
                              onClick={() => this.changeRange(-1, C_Id)}
                              id="pagPrevId"
                            />
                          ...
                          <Tab.Container id="list-group-tabs-example">
                            <ListGroup className="mt-2" id="lgId2">
                              ...
                              {this.state.totalLectures
                                ...
                                ?.slice(
                                  this.state.range * view,
                                  this.state.range * view + view
                                )
                                ?.map((c) => (
                                  <ListGroup.Item
                                    action
