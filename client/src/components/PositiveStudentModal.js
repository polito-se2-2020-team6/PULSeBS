...
class PositiveStudentModal extends React.Component {
  ...
  generateReport = async () => {
    ...
    const responseCsv = await API.getCTReport(
      this.props.positiveSTD.userId,
      this.state.value.value
    );
    const filename = fnGetFileNameFromContentDispostionHeader(
      responseCsv.headers.get("content-disposition")
    );
    const blob = await responseCsv.blob();
    const newBlob = new Blob([blob], {
      type: responseCsv.headers.get("content-type"),
    });
